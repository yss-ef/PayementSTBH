 ...
 */
class pdf_sponge_stbh extends pdf_sponge
{
	/**
...
{
    parent::__construct($db); // On appelle le constructeur du parent (sponge)

    $this->name = "sponge_stbh";
    $this->description = "Modèle STBH (base Sponge) avec taxe spéciale";
}

// On réécrit la fonction qui affiche le bloc des totaux.
protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs, $outputlangsbis)
{
    // On charge les attributs supplémentaires de la facture
    $object->fetch_optionals();

    // Si la facture est internationale...
    if (!empty($object->array_options['options_is_international'])) {

        $tax_name = 'Taxe Spéciale'; // Valeur par défaut
        if (count($object->lines) > 0) {
            $first_line = $object->lines[0];
            $first_line->fetch_optionals();
            if (!empty($first_line->array_options['options_tsr_name_line'])) {
                $tax_name = $first_line->array_options['options_tsr_name_line'];
            }
        }

        // On modifie temporairement la traduction de "TotalVAT"
        $outputlangs->strings['TotalVAT'] = 'Montant ' . $tax_name;
    }

    // On appelle la fonction originale de "sponge" pour qu'elle fasse le dessin
